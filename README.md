...
支持应用、文件、文件夹、浏览器 + 网址一键打开。

## 功能

- 🔍 **双源扫描** — Start Menu .lnk + Shell.AppsFolder COM，覆盖桌面应用和 UWP
- 📁 **自定义组合** — 创建应用分组（如"上班""游戏"），每个分组可添加任意应用
- ✏️ **手动添加** — 扫不到的应用支持自定义添加，类型下拉框切换应用/文件/文件夹
- 📂 **文件/文件夹** — 支持打开任意文件（文档/图片/音乐）和文件夹，系统关联程序打开
- 🔄 **检查更新** — 关于对话框一键检测 GitHub 最新版本
...
| **手动添加** | 选中分组 → 下方 [自定义添加] → 选择类型（应用/文件/文件夹）→ 浏览路径 |
| **管理员运行** | 勾选条目右侧「管理员」列 → 启动时 UAC 提权 |
| **文件/文件夹** | 自定义添加时类型选「文件」或「文件夹」，启动时用系统关联程序打开 |
| **拖拽排序** | 左侧分组列表直接拖动调整顺序 |
| **设置网址** | 表格中双击 URL 列，输入完整网址 |
| **检查更新** | 右键托盘 → 关于 BatchGo → [检查更新] |
| **重新扫描** | 配置面板右上角 [🔄 重新扫描] |
